...
cli = typer.Typer(help="This identifies networks of individuals who share an IBD segment around a genomic region of interest")
...
@cli.command()
def gene_analysis(
    binary_file: str = typer.Argument(..., help="This is the binary file that has the genotyping information"),
    output: str = typer.Option("./", help="file directory to output all output files into"),
    pop_info_file: Optional[str] = typer.Argument(None,help="filepath to the file that has all the ancestry information of the individuals in the binary file"),
    pop_code: Optional[str] = typer.Argument(None,help="Optional population code for the data to be subset into. This is based on 1000 genomes"),
    range_str: Optional[str] = typer.Argument(None,help="Genomic start position to extract a range of snps from. This is used for a gene approach"),
    variant_file: Optional[str] = typer.Argument(None,help="This is the file that list wariants of interest and there genomic information"),
    range_end: Optional[str] = typer.Argument(None,help="Genomic end position to extract a range of snps from. This is used for a gene approach"),
    MIN_CM: int = typer.Option(3, help="minimum centimorgan threshold that will be used to find the minimum size of shared IBD segments between pairs", prompt=True, callback=utility_scripts.check_provided_integer),
    THREADS: int = typer.Option(3, help="number of threads to parallelize the process to. This value has to be at least 3", prompt=True, callback=utility_scripts.check_provided_integer),
    MAF_THRESHOLD: float = typer.Option(0.05, help="maf frequency threshold where only variants below the threshold will be kept", prompt=True, callback=utility_scripts.check_provided_maf),
    IBD_programs: str = typer.Option(..., help="Enter the ibd programs whose output was used in the analysis. Enter each program separated by a space", prompt=True)
):
    ...
    utility_scripts.record_user_arguments(logger, 
        {
    ...
    logger.info(
        f"Writing the results of the carrier analysis called: {''.join([output, 'carrier_analysis_output/'])}"
    )
    ...
    carrier_analysis_scripts.determine_maf(
        "".join([output, "carrier_analysis_output/"]),
        "".join([output, "plink_output_files/"]), pop_info_file,
        pop_code, output)
    ...
    variants_above_threshold_tuple: tuple = carrier_analysis_scripts.check_mafs(
        "".join([
            output, "carrier_analysis_output/", "allele_frequencies.txt"
        ]), THRESHOLD)
    ...
    ibd_file_dict: dict = pre_shared_segments_analysis_scripts.shared_segment_detection.build_file_dict(gathered_file_dict["ibd_pair_file_list"], IBD_programs.split(" "), "gene")
    ...
    combine_output(
        gathered_file_dict, ibd_file_dict, IBD_search_output_files, "gene",THREADS, analysis_files)
    ...
    create_network_scripts.create_networks(
        os.path.join(IBD_search_output_files, "pairs/"), network_dir, "gene", os.path.join(IBD_search_output_files, "confirmed_carriers.txt"))
...
def phenotype_analysis(
    ...
    pop_info_file: Optional[str] = typer.Argument(None, help="filepath to the file that has all the ancestry information of the individuals in the binary file"),
    pop_code: Optional[str] = typer.Argument(None, help="Optional population code for the data to be subset into. This is based on 1000 genomes"),
    pheno_gmap: str = typer.Argument(None, help="filepath to the file that contains information about the chromosome of interest and the start and end position of the gene of interest"),
    pheno_carriers: str = typer.Argument(..., help="Filepath to a file that contains information about grids identified as carriers based on phenotype"),
    MIN_CM: int = typer.Option(3, help="minimum centimorgan threshold that will be used to find the minimum size of shared IBD segments between pairs", prompt=True, callback=utility_scripts.check_provided_integer),
    THREADS: int = typer.Option(3, help="number of threads to parallelize the process to. This value has to be at least 3", prompt=True, callback=utility_scripts.check_provided_integer),
    IBD_programs: str = typer.Option(..., help="Enter the ibd programs whose output was used in the analysis. Enter each program separated by a space", prompt=True)
):
    ...
    ibd_file_dict: dict = pre_shared_segments_analysis_scripts.shared_segment_detection.build_file_dict(gathered_file_dict["ibd_pair_file_list"], IBD_programs.split(" "), "phenotype")
    ...
    combine_output(
        gathered_file_dict, ibd_file_dict, IBD_search_output_files, "phenotype", THREADS,
        analysis_files)
    ...
    create_network_scripts.create_networks(
        os.path.join(IBD_search_output_files, "pairs/"), network_dir, "phenotype", os.path.join(IBD_search_output_files, "confirmed_carriers.txt"))
