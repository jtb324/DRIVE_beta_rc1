...
def singleVariantAnalysis(recodeFile: list, write_path: str, reformat: bool,
                          fileName: str, pop_info: str, pop_code: str):
    ...
    for file_tuple in recode_file_list:
        ...
        if var_dict_reformat:
            ...
            var_reformat_df.to_csv("".join([
                reformat_directory, "/", file_prefix,
                ".single_var_list_reformat.csv"
            ]),
                                   index=False)

            logger.info(
                'The list of IIDs for each probe id were written to this filepath, {}'
                .format(
        ...
        totalVariantIDList(total_id_set, write_path, file_tuple[1])
